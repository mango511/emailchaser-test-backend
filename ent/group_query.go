...
func (gq *GroupQuery) QueryGroup() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, group.GroupTable, group.GroupColumn),
		)
...
func (gq *GroupQuery) WithGroup(opts ...func(*UserQuery)) *GroupQuery {
	query := (&UserClient{config: gq.config}).Query()
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withGroup; query != nil {
		if err := gq.loadGroup(ctx, query, nodes,
			func(n *Group) { n.Edges.Group = []*User{} },
			func(n *Group, e *User) { n.Edges.Group = append(n.Edges.Group, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GroupQuery) loadGroup(ctx context.Context, query *UserQuery, nodes []*Group, init func(*Group), assign func(*Group, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.GroupColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.group_group
		if fk == nil {
			return fmt.Errorf(`foreign-key "group_group" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "group_group" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
